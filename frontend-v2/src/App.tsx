...
export const Dashboard: React.FC = () => {
  const { studentId, username, selectedLine, setStudentId, setCredentials } = useAuthStore();
  ...
  return (
    <div className="min-h-screen relative overflow-x-hidden text-slate-800 antialiased font-sans">
      {/* 1. Dynamic Animated Running Train & Moiré Interference Background */}
      <RunningTrainBackground />

      {/* 2. Top Network Signal & Live Telemetry Ticker (Frosted Light Slate) */}
      <div className="relative z-20 bg-slate-900/90 backdrop-blur-md text-slate-200 text-xs px-4 py-2 border-b border-slate-800/80 shadow-xs">
        <div className="max-w-7xl mx-auto flex flex-wrap items-center justify-between gap-3">
          ...
          <div className="hidden lg:flex items-center gap-2 text-[11px] font-mono text-slate-300">
            ...
            <span className="bg-slate-800 px-2 py-0.5 rounded border border-slate-700/80">IST {currentTime}</span>
          </div>
        </div>
      </div>

      {/* 3. Executive Command Header with Glassmorphism and Moiré Frame */}
      <header className="relative z-20 bg-white/85 backdrop-blur-xl border-b border-slate-200/90 sticky top-0 px-4 sm:px-8 py-3.5 shadow-xs transition-all">
        <div className="max-w-7xl mx-auto flex flex-wrap justify-between items-center gap-4">
          <div className="flex items-center gap-3.5">
            {/* Logo Emblem with active Line Glow */}
            <div className="w-11 h-11 rounded-2xl bg-gradient-to-tr from-slate-900 via-slate-800 to-indigo-900 flex items-center justify-center text-white shadow-md shadow-slate-300/60 border border-slate-700/40 relative overflow-hidden group">
            ...
            <div>
              <div className="flex items-center gap-2">
                ...
                <span className="text-[10px] bg-slate-900 text-white font-mono font-bold px-2 py-0.5 rounded-md uppercase tracking-wider shadow-2xs">
                  OPS-DESK
                </span>
                <span className="hidden md:inline-block font-hand text-base text-rose-600 font-bold -rotate-2">
                  Today's m-Indicator Master Schedule ✍️
                </span>
              </div>
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="hidden sm:flex items-center gap-1.5 px-3 py-1.5 bg-slate-100 rounded-xl border border-slate-200 text-xs font-mono font-bold text-slate-700 shadow-2xs">
              ...
              <span>ACTIVE: {selectedLine === "ALL" ? "MULTI-LINE" : `${selectedLine} CORRIDOR`}</span>
            </div>

            {/* Portal Configuration Trigger */}
            <button
              onClick={() => setShowConfig(!showConfig)}
              className="flex items-center gap-2 text-xs bg-slate-900 hover:bg-slate-800 text-white px-4 py-2.5 rounded-xl font-bold transition-all shadow-sm active:scale-95 border border-slate-800"
            >
              <KeyRound className="w-3.5 h-3.5 text-indigo-300" />
              <span>{showConfig ? "Close Credentials" : studentId ? `Roll: ${studentId}` : "Set Portal Login"}</span>
            </button>
          </div>
        </div>
      </header>

      {/* 4. Main Dashboard Workspace with Light Grey Layout */}
      <main className="relative z-10 max-w-7xl mx-auto p-4 sm:p-6 lg:p-8 space-y-7">
        {/* Handwritten Blueprint Sticky Memo */}
        <div className="flex flex-wrap items-center justify-between gap-3 bg-gradient-to-r from-amber-50/80 via-white/80 to-slate-50/80 backdrop-blur-md p-3.5 rounded-2xl border border-amber-200/60 shadow-xs">
          <div className="flex items-center gap-3">
          ...
          <div className="font-mono text-[11px] text-slate-400 hidden sm:block">
            STAMP: CR-ENG-VERIFIED // <span className="font-hand text-slate-600 font-bold text-sm">Approved ✍️</span>
          </div>
        </div>

        {/* Expandable Portal Configuration Form */}
        {showConfig && (
          <form
            onSubmit={handleSave}
            className="bg-white/95 backdrop-blur-xl p-6 rounded-3xl border border-slate-200/90 shadow-xl transition-all relative overflow-hidden"
          >
            {/* Top decorative moiré edge */}
            ...
            <div className="flex items-center justify-between mb-4">
              <div>
                ...
                <p className="text-xs text-slate-500 font-medium">
                  Connect your University of Mumbai ERP credentials for automated attendance sync & exam tracking
                </p>
              </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <div>
                <label className="text-xs font-bold text-slate-600 uppercase tracking-wider flex items-center justify-between">
                  ...
                  <span className="font-hand text-xs text-slate-400">MU ERP Login</span>
                </label>
                <input
                  type="text"
                  required
            ...
            <div className="mt-5 pt-4 border-t border-slate-100 flex flex-wrap justify-between items-center gap-3">
              <div className="flex items-center gap-2 text-xs text-slate-500 font-mono">
                ...
                <span>ADAPTER: University of Mumbai (MU_STANDARD) ERP</span>
              </div>
              ...
              <div className="flex items-center gap-2">
                ...
                >
                  <CheckCircle2 className="w-4 h-4" /> Save Credentials & Sync
                </button>
              </div>
            </div>
          </form>
